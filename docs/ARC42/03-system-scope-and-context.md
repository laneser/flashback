...
graph TB
    ...
    WIFI_FL[💡 WiFi 閃光燈<br/>選配]
    BLE_FL[💡 BLE 閃光燈<br/>選配]
    ESP[🔌 ESP32<br/>選配]
    RF_FL[💡 2.4GHz 閃光燈<br/>選配]
    MAIN -.->|WiFi| WIFI_FL
    MAIN -.->|Bluetooth| BLE_FL
    MAIN -.->|BT/WiFi| ESP
    ESP -.->|2.4GHz| RF_FL
    WIFI_FL -.->|⚡ 強力閃光| RIDER
    BLE_FL -.->|⚡ 強力閃光| RIDER
...
| 從裝置 | — | WebSocket 觸發指令 |
| WiFi 閃光燈（選配） | — | WiFi 觸發指令 |
| BLE 閃光燈（選配） | — | Bluetooth 觸發指令 |
| ESP32 + 2.4GHz 閃光燈（選配） | — | BT/WiFi → ESP32 → 2.4GHz 控制訊號 |
| Telegram（選配） | — | 觸發通知訊息 |
...
graph LR
    ...
    subgraph 外接閃光 選配
        WIFI_FL2[WiFi 閃光燈]
        BLE_FL2[BLE 閃光燈]
        ESP_C[ESP32 Bridge] --> RF_FL2[2.4GHz 閃光燈]
    end
    ...
    TRIGGER -.->|WiFi| WIFI_FL2
    TRIGGER -.->|BLE| BLE_FL2
    TRIGGER -.->|BT/WiFi| ESP_C
    LOG -.->|HTTP| TG_BOT
```
...
| 外接閃光 WiFi（選配） | WiFi 直連 | 廠商協定 | 閃光燈內建 WiFi |
| 外接閃光 BLE（選配） | Bluetooth Low Energy | GATT | 閃光燈內建 BLE |
| 外接閃光 2.4GHz（選配） | ESP32 橋接 | Phone→BT/WiFi→ESP32→2.4GHz | 需 ESP32 轉接 |
